 ...
 */
function getDispatcherObject(target: any /* TODO : Targetable [bug since TS2.9] */, methodName: string | symbol): Dispatcher {
...
export function after<F extends GenericFunction<T>, T>(
	...
	if (typeof joinPointOrTarget === 'function') {
		return afterJoinPoint(<any>joinPointOrTarget, <JoinPointAfterAdvice<T>>methodNameOrAdvice);
	} else {
...
export function around<F extends GenericFunction<T>, T>(
	...
	if (typeof joinPointOrTarget === 'function') {
		return aroundJoinPoint(<any>joinPointOrTarget, <JoinPointAroundAdvice<T>>methodNameOrAdvice);
	} else {
...
export function before<F extends GenericFunction<T>, T>(
	...
	if (typeof joinPointOrTarget === 'function') {
		return beforeJoinPoint(<any>joinPointOrTarget, <JoinPointBeforeAdvice>methodNameOrAdvice);
	} else {
