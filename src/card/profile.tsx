...
import { isLinkOrImage, getActorName } from '../common/activityPubUtil';
...
export const Profile = factory(function Profile({ children, properties, middleware: { icache, theme } }) {
	...
	icache.getOrSet('preferredUsername', getActorName(properties()));
...
${eCount < 2 ? '' : (eCount === 2 ? ' and 1' : `& ${eCount-1} others`)}`;
	...
	const avatarFromStr = () => {
		const s = icache.get('preferredUsername')||'?';
		const w = words(s);
				...
				<div classes={themedCss.metaWrapper}>
					{petName ?
						...
						(icache.get('preferredUsername') ?
							<h3 classes={themedCss.actorName}>{icache.get('preferredUsername')}</h3> :
							''
						)
					}
				</div>
