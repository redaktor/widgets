(function (factory) {
    ...
    //
    var miscSearch = ['#Value of #Month'];
    var preps = 'in|by|before|for|during|at|on|until|after|of|within|all';
    var miscDays = { today: 0, tomorrow: 1, yesterday: -1 };
    var weekDays = {
    ...
    exports.dates = { miscSearch: miscSearch, preps: preps, miscDays: miscDays, weekDays: weekDays, months: months };
    function date() {
        var _this = this;
        //console.log('.DATE', this.out('text'), this.match);
        // ensure a year is approximately typical for common years
        // please change in one thousand years
        var tagYear = function (v, reason) {
            v.list.forEach(function () {
                var num = parseInt(_this.terms[0].normal, 10);
                if (num && num > 1000 && num < 3000) {
                    _this.terms[0].tag('Year', reason);
                }
            });
        };
        // same, but for less-confident values
        var tagYearSafer = function (v, reason) {
            v.list.forEach(function () {
                var num = parseInt(_this.terms[0].normal, 10);
        ...
        if (this.has('#Value')) {
            ...
            // may twenty five
            var vs = this.match('#TextValue #TextValue');
        ...
        if (this.has('#Cardinal')) {
            var v = this.match("#Date #Value #Cardinal").lastTerm;
            if (v.found) {
        ...
        // fix over-greedy
        if (this.has('#Date')) {
            var date_1 = this.match('#Date+').splitOn('Clause');
            if (date_1.has('(#Year|#Time)') === false) {
                // 12 february 12
                date_1.match('#Value (#Month|#Weekday) #Value').lastTerm.untag('Date');
            }
        }
        return this;
    }
