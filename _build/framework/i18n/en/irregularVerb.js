(function (factory) {
    ...
    var Verb = {
        conjugations: conjugations, suffixes: suffixes,
        aux: function (ts) {
        ...
        // had walked
        isPerfect: function (ts) { return ts.match('^(had|have) #PastTense'); },
        /* TODO FIXME */
        isImperative: function (ts) { return ts; },
        isConditional: function (ts) { return ts; },
        toBe: function (isPlural, isNegative) {
            ...
            if (isNegative) {
                ['PastTense', 'PresentTense', 'Infinitive'].forEach(function (k) { o[k] += ' not'; });
                ['PerfectTense', 'Pluperfect'].forEach(function (k) { o[k] = "not " + o[k]; });
        ...
        correctConjugate: function (o, isNegative) {
            if (isNegative === void 0) { isNegative = false; }
            // apply negative
            if (isNegative) {
    ...
    for (var inf in Verb.conjugate) {
        var o = {};
        for (var k in Verb.conjugate[inf]) {
            if (!!Verb.conjugate[inf][k].length) {
                var newTag = Verb.tenseMap[k];
    ...
    for (var k in compact) {
        for (var i = 0; i < compact[k].length; i++) {
            Verb.suffixRules[compact[k][i]] = k;
        }
    }
    var hasY = /[bcdfghjklmnpqrstvwxz]y$/;
    Verb.generic = {
        Gerund: function (s) { return (s.charAt(s.length - 1) === 'e' ? s.replace(/e$/, 'ing') : s + "ing"); },
        PresentTense: function (s) {
            if (s.charAt(s.length - 1) === 's') {
                return s + "es";
            }
            return (hasY.test(s) === true) ? s.slice(0, -1) + "ies" : s + "s";
        },
        PastTense: function (s) {
