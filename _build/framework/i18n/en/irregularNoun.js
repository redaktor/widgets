(function (factory) {
    ...
    */
    ...
    var Noun = {
        irregulars: irregulars, singularRules: singularRules, pluralRules: pluralRules,
        pluralIndicators: singularRules.map(function (a) { return a[0]; }).concat([
            /ives$/i, /(^f)ves$/i, /(lr)ves$/i
        ]),
        singularIndicators: pluralRules.map(function (a) { return a[0]; }).concat([
            /(rl)f$/i, /(?:(^f)fe|(lr)f)$/i
        ]),
        knownPlural: { i: 1, he: 1, she: 1, we: 1, they: 1 },
        prep: /([a-z]*) (of|in|by|for) [a-z]/,
        prepStart: /^([a-z]*) (of|in|by|for) [a-z]/,
        isPluralFallback: function (s) { return (s.length > 3 && !/s$/.test(s) === true && /ss$/.test(s)); },
        makeArticle: function (t) {
            // chooses an indefinite aricle 'a/an' for a word
            var irregArticle = {
                hour: 'an', heir: 'an', heirloom: 'an', honest: 'an', honour: 'an', honor: 'an', uber: 'an'
            };
            // pronounced letters of acronyms that get a 'an'
            var anAcronyms = { a: 1, e: 1, f: 1, h: 1, i: 1, l: 1, m: 1, n: 1, o: 1, r: 1, s: 1, x: 1 };
            //'a' regexes
            var aRegexps = [/^onc?e/i, /^u[bcfhjkqrstn][aeiou]/i, /^eul/i];
            var s = t.normal;
            ...
            //'a' regexes
            for (var i = 0; i < aRegexps.length; i++) {
